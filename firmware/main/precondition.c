...
typedef struct {
    uint8_t cmd; // 1: toggle, 2: start, 3: stop
    int8_t mode; // 0: ONCE, 1: CONTINUOUS, 2: PERSISTENT, -1: unchanged
} precon_req_t;
...
static void precondition_global_tick(sm_t *sm) {
    ...
    // web UI toggle / automation requests are handled here, on the precondition task, so
    // the precondition state machine stays single-writer
    precon_req_t req = {0};
    if (precondition_toggle_queue != NULL
            && xQueueReceive(precondition_toggle_queue, &req, 0) == pdTRUE) {
        if (req.mode >= 0 && req.mode <= 2) {
            precon_config.mode = req.mode;
        ...
        if (req.cmd == 1) {
            ...
            if (sm_in(&precon_sm, &S_IDLE)) {
                sm_transition(&precon_sm, &S_REQUESTED);
            } else if (sm_in(&precon_sm, &S_MANAGED)) {
                sm_transition(&precon_sm, &S_START_BURST);
            }
        } else if (req.cmd == 3) {
            if (!sm_in(&precon_sm, &S_IDLE)) {
                sm_transition_arg(&precon_sm, &S_STOPPING, STOP_REASON_USER);
            }
        }
    }
...
void precondition_toggle_request(void) {
    ...
    precon_req_t req = { .cmd = 1, .mode = -1 };
    xQueueOverwrite(precondition_toggle_queue, &req);
}
...
void precondition_start_request(int mode) {
    ...
    precon_req_t req = { .cmd = 2, .mode = (int8_t)mode };
    xQueueOverwrite(precondition_toggle_queue, &req);
}
...
void precondition_stop_request(void) {
    ...
    precon_req_t req = { .cmd = 3, .mode = -1 };
    xQueueOverwrite(precondition_toggle_queue, &req);
}

void precondition_execute_action(const char *mode, const char *action) {
    if (action != NULL && (strcasecmp(action, "stop") == 0 || strcasecmp(action, "cancel") == 0 || strcasecmp(action, "off") == 0)) {
        precondition_stop_request();
        return;
    }
    if (action != NULL && strcasecmp(action, "toggle") == 0) {
        precondition_toggle_request();
        return;
    }
    if (mode != NULL && (strcasecmp(mode, "cancel") == 0 || strcasecmp(mode, "off") == 0)) {
        precondition_stop_request();
        return;
    }
    ...
    int m = PERSISTENT;
    if (mode != NULL) {
        if (strcasecmp(mode, "continuous") == 0) m = CONTINUOUS;
        else if (strcasecmp(mode, "once") == 0 || strcasecmp(mode, "single") == 0) m = ONCE;
        else if (strcasecmp(mode, "persistent") == 0) m = PERSISTENT;
    }
    precondition_start_request(m);
}
...
void precondition_init(void) {
    ...
    precondition_toggle_queue = xQueueCreate(2, sizeof(precon_req_t));
